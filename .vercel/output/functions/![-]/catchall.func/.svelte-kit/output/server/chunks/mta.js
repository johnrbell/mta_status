...
function processAlerts(feedData) {
  ...
  for (const e of feedData.entity) {
    ...
    const createdAt = mercury.created_at ? new Date(mercury.created_at * 1e3) : null;
  ...
  for (const route of allRoutes) {
    ...
    const status = alerts.length > 0 ? mapStatus(pickWorstStatus(alerts.map((a) => a.type))) : "all good.";
    trains[sortOrder[route]] = { route, statusDetails: { statusSummary: status }, alerts };
  }
  return { trains, cacheTime: /* @__PURE__ */ new Date() };
}
const MTA_URL = "https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/camsys%2Fsubway-alerts.json";
const CACHE_TTL = 300;
let cachedData = null;
async function fetchTrainData() {
  if (cachedData && (Date.now() - cachedData.cacheTime.getTime()) / 1e3 < CACHE_TTL) {
    return cachedData;
  }
